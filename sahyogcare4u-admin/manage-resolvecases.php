...
include('lock.php');
$where = [];
$sql_count = "SELECT count(*) as total FROM `resolvecase` WHERE 1=1 ";
...
$sql   = "SELECT * FROM `resolvecase` WHERE 1=1 ORDER BY id DESC LIMIT :start,:limit";
               ...
               <div class="col-md-12">
                  <div class="panel">
                     <div class="panel-heading">
                        <h4 class="panel-title">Manage Resolve Cases</h4>
                     </div>
                                 ...
                                 ?>
                                    <tr>
                                       <td><?= $i++; ?></td>
                                        ...
                                        <td><?= $row['resolvecasename'] ?></td>
                                        <td><?= $row['age'] ?></td>
                                       <td>
                                          <?php if (!empty($row['resolvecaseimage'])): ?>
                                             <img src="<?= BASE_URL . $row['resolvecaseimage']; ?>" alt="Resolve case" width="50" height="50"><br>
                                             <a href="<?= BASE_URL . $row['resolvecaseimage']; ?>" target="_blank">View </a>
                                          <?php else: ?>
                                       ...
                                       <td><?= $row['tstp'] ?></td>
                                       <td>
                                          <a href="edit-resolvecase.php?id=<?= base64_encode($row['id']) ?>"
                                             onclick="return confirm('Are you sure you want to update this data?');">
                                             Update
                                          </a>
                                       </td>
                                       <td>
                                          <a href="delete-resolvecase.php?id=<?= base64_encode($row['id']) ?>"
